...
%Umožnuje vkládání zdrojových kódu%
\usepackage{listings}

%Umožňuje barvy - kvuli kodum%
\usepackage{color}
...
Pro RGB, HSV i LAB použijeme barevnou hloubku 16 bitů na kanál histogramu v jejich příslušném barevném prostoru. K určení příslušné vzdálenosti se můžeme setkat se čtyřmi měřítky vzdálenosti pro histogramy a rozdělení (K L-divergence, $\chi^2$ statistika, L1 - vzdálenost a L2 - vzdálenost). Na RGB a HSV je nejlépší použít L1 zatímco pro LAB je nejvhodnější K L-divergence.  
...
Systém byl navržen jako modulový a to z důvodu snadné obměny některé z častí, což je výhodné zejména pokud by jsme potřebovali například spočítat vzdálenosti vektorů podle jiného algoritmu. 
...
Zpracování výsledků probíhá jako porovnání anotací přidělených člověkem s anotacemi přiděleny klasifikátorem. Máme sadu obrázků $r_J$ porovnáme ji tak aby se dospělo k podmiňovacímu rozdělení $P(w|r_J)$. Vezmeme top 5 slov z tohoto rozdělení a zavoláme na ně automatickou anotaci obrázku v dotazu. Pak následující mi spočítáme anotaci recall a precision pro každé slovo v  testovacím sadě.  Recall je počet správně anotovaných obrázků v testovací sadě děleno počtem snímků, které mají toto slovo v anotaci přidělené člověkem. Precision je počet správně anotovaných obrázků děleno celkovým počtem anotovaných obrázků. 
